...
#error "LV_FONT_MONTSERRAT_14 is required for pgs_benchmark. Enable it in lv_conf.h."
#endif

#if LV_FONT_MONTSERRAT_24 == 0
#error "LV_FONT_MONTSERRAT_24 is required for pgs_benchmark. Enable it in lv_conf.h."
#endif
...
void pgs_benchmark(lv_group_t * group, void (*key_cb)(uint32_t keycode))
{
    scene_act = 0;
    ...
    fps_title = lv_label_create(lv_layer_top());
    lv_obj_set_style_bg_opa(fps_title, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(fps_title, lv_color_white(), 0);
    lv_obj_set_style_text_color(fps_title, lv_color_black(), 0);
    lv_obj_set_width(fps_title, lv_pct(100));
    ...
    lv_subject_add_observer_obj(&disp->perf_sysmon_backend.subject, sysmon_perf_observer_cb, fps_title, NULL);
#if LV_USE_PERF_MONITOR_LOG_MODE
    lv_obj_add_flag(fps_title, LV_OBJ_FLAG_HIDDEN);
#endif
#else
    lv_label_set_text(fps_title, "LV_USE_PERF_MONITOR is not enabled");
#endif
}
...
static void sysmon_perf_observer_cb(lv_observer_t * observer, lv_subject_t * subject)
{
    ...
    lv_label_set_text_fmt(label,
                          "    %s"
                          "%" LV_PRIu32 " FPS\n    refr.%" LV_PRIu32 "ms = %" LV_PRIu32 " render + %" LV_PRIu32 " flush",
                          scene_name, info->calculated.fps,
...
static void summary_create(void)
{
    ...
    lv_group_add_obj(ui_group, table);

    lv_obj_set_style_border_width(table, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_pad_ver(table, 6, LV_PART_ITEMS);

    lv_obj_add_event_cb(table, table_draw_task_event_cb, LV_EVENT_DRAW_TASK_ADDED, NULL);
    lv_obj_add_event_cb(table, key_event_cb, LV_EVENT_KEY, NULL);
    ...
    int32_t col_w = lv_obj_get_content_width(table) / 2;

    lv_table_set_column_width(table, 0, col_w);
    lv_table_set_column_width(table, 1, col_w / 2);
    lv_table_set_column_width(table, 2, col_w / 2);
    ...
    for(i = 0; scenes[i].create_cb; i++) {
        ...
        if(scenes[i].measurement_cnt <= 1) {
            ...
            lv_table_set_cell_value_fmt(table, i + 2, 1, "%" LV_PRIu32, scenes[i].fps_avg / cnt);

            uint32_t render_time = scenes[i].render_avg_time / cnt;
            uint32_t flush_time  = scenes[i].flush_avg_time / cnt;
            lv_table_set_cell_value_fmt(table, i + 2, 2, "%" LV_PRIu32 "+%" LV_PRIu32, render_time, flush_time);
    ...
    if(valid_scene_cnt < 1) {
        ...
        lv_table_set_cell_value_fmt(table, 1, 1, "%" LV_PRIu32, total_avg_fps / valid_scene_cnt);

        uint32_t render_time = total_avg_render_time / valid_scene_cnt;
        uint32_t flush_time  = total_avg_flush_time / valid_scene_cnt;
        lv_table_set_cell_value_fmt(table, 1, 2, "%" LV_PRIu32 "+%" LV_PRIu32, render_time, flush_time);
        /* csv log */
