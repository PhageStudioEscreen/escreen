...
static void nes_frame_throttle(void)
{
  static uint64_t next_deadline_us = 0;
  struct timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  uint64_t now_us = (uint64_t)ts.tv_sec * 1000000ULL + (uint64_t)ts.tv_nsec / 1000ULL;

  if (next_deadline_us == 0) {
    next_deadline_us = now_us + 16667ULL;
    return;
  }

  if (now_us < next_deadline_us) {
    usleep((useconds_t)(next_deadline_us - now_us));
    now_us = next_deadline_us;
  } else if (now_us - next_deadline_us > 50000ULL) {
    next_deadline_us = now_us;
  }

  next_deadline_us += 16667ULL;
}
 ...
 */
  ...
  // Clear audio buffers before starting game
  ...
  for (;;)
  {
    ...
    // Small delay to slow down CPU emulation
#if 0
    usleep(3);
